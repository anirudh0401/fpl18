...
function getCoord(e) {
	...
	div.textContent = enableAnimate.value.substr(0, carPos);
	span.textContent = enableAnimate.value.substr(carPos) || '.';
	div.appendChild(span);

	enableAnimateCoords = getPosition(enableAnimate);							//console.log("enableAnimateCoords.x: " + enableAnimateCoords.x + ", enableAnimateCoords.y: " + enableAnimateCoords.y);
	...
	caretPos = caretCoords.x + enableAnimateCoords.x;					//console.log("caretPos: " + caretPos);
	...
	var eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, enableAnimateCoords.x + caretCoords.x, enableAnimateCoords.y + 25);
	var eyeLX = Math.cos(eyeLAngle) * eyeMaxHorizD;
	var eyeLY = Math.sin(eyeLAngle) * eyeMaxVertD;
	var eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, enableAnimateCoords.x + caretCoords.x, enableAnimateCoords.y + 25);
	var eyeRX = Math.cos(eyeRAngle) * eyeMaxHorizD;
	var eyeRY = Math.sin(eyeRAngle) * eyeMaxVertD;
	var noseAngle = getAngle(noseCoords.x, noseCoords.y, enableAnimateCoords.x + caretCoords.x, enableAnimateCoords.y + 25);
	var noseX = Math.cos(noseAngle) * noseMaxHorizD;
	var noseY = Math.sin(noseAngle) * noseMaxVertD;
	var mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, enableAnimateCoords.x + caretCoords.x, enableAnimateCoords.y + 25);
...
function onenableAnimateInput(e) {
	getCoord(e);
	var value = e.target.value;
	curenableAnimateIndex = value.length;

	// very crude enableAnimate validation for now to trigger effects
	if (curenableAnimateIndex > 0) {
...
function onenableAnimateFocus(e) {
	e.target.parentElement.classList.add("focusWithText");
	getCoord();
}

function onenableAnimateBlur(e) {
	if (e.target.value == "") {
...
function onenableHideAnimateFocus(e) {
	coverEyes();
}

function onenableHideAnimateBlur(e) {
	uncoverEyes();
}
...
function initialize() {
	...
	enableAnimate = document.querySelector('#enableAnimate'), enableHideAnimate = document.querySelector('#enableHideAnimate'), mySVG = document.querySelector('.svgContainer'), armL = document.querySelector('.armL'), armR = document.querySelector('.armR'), eyeL = document.querySelector('.eyeL'), eyeR = document.querySelector('.eyeR'), nose = document.querySelector('.nose'), mouth = document.querySelector('.mouth'), mouthBG = document.querySelector('.mouthBG'), mouthSmallBG = document.querySelector('.mouthSmallBG'), mouthMediumBG = document.querySelector('.mouthMediumBG'), mouthLargeBG = document.querySelector('.mouthLargeBG'), mouthMaskPath = document.querySelector('#mouthMaskPath'), mouthOutline = document.querySelector('.mouthOutline'), tooth = document.querySelector('.tooth'), tongue = document.querySelector('.tongue'), chin = document.querySelector('.chin'), face = document.querySelector('.face'), eyebrow = document.querySelector('.eyebrow'), outerEarL = document.querySelector('.earL .outerEar'), outerEarR = document.querySelector('.earR .outerEar'), earHairL = document.querySelector('.earL .earHair'), earHairR = document.querySelector('.earR .earHair'), hair = document.querySelector('.hair');
	caretPos, curenableAnimateIndex, screenCenter, svgCoords, eyeMaxHorizD = 20, eyeMaxVertD = 10, noseMaxHorizD = 23, noseMaxVertD = 10, dFromC, eyeDistH, eyeLDistV, eyeRDistV, eyeDistR, mouthStatus = "small";

	enableAnimate.addEventListener('focus', onenableAnimateFocus);
	enableAnimate.addEventListener('blur', onenableAnimateBlur);
	enableAnimate.addEventListener('input', onenableAnimateInput);
	enableHideAnimate.addEventListener('focus', onenableHideAnimateFocus);
	enableHideAnimate.addEventListener('blur', onenableHideAnimateBlur);
